...
fun main(args: Array<String>) {
    ...
    if (input.endsWith(".solution")) {
        File(output ?: "$input.kts").writeText(DslGenerator.toDsl(SolutionParser.parse(File(input).inputStream().asInput())))
    } else if (input.endsWith(".solution.kts")) {
        when (val result = DslCompiler.fromDsl(File(input).readText())) {
            is ResultWithDiagnostics.Success -> SolutionParser.write(result.value, File(output ?: input.removeSuffix(".kts")).outputStream().asOutput())
